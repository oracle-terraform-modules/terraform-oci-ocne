...
output "ip_address" {
  ...
  value       = var.instance_count > 0 ? oci_load_balancer_load_balancer.lb[0].ip_address_details[0].ip_address: ""
}
...
output "endpoint" {
  ...
  value       = var.instance_count > 0 ? "${oci_load_balancer_load_balancer.lb[0].ip_address_details[0].ip_address}:${var.port}" : ""
}

output "load_balancer_ocid" {
  description = "OCID of the load balancer."
  value       = var.instance_count > 0 ? oci_load_balancer_load_balancer.lb[0].id : ""
}
